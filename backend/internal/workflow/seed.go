...
func MinimumLoopDefinition(id, createdBy string, now time.Time) (domain.WorkflowDefinition, error) {
	...
	nodes := []domain.NodeDefinition{
		...
		{ID: "blueprint-ai", Name: "Generate blueprint proposal", Type: domain.NodeAITransform, InputSchema: envelope, OutputSchema: envelope, AITransform: &domain.AITransformNodeConfig{JobType: "decompose_pages", ModelPolicy: "project-default", OutputSchemaVersion: "blueprint-proposal/v1", MaxAttempts: 3, Timeout: 5 * time.Minute}},
		...
		{ID: "pages", Name: "Create page delivery slices", Type: domain.NodeFanOut, InputSchema: envelope, OutputSchema: envelope, FanOut: &domain.FanOutNodeConfig{ItemsPath: "/payload/pages", SliceKeyPath: "/id", MergeNodeID: "pages-merged", MaxParallel: 4}},
		...
		{ID: "quality", Name: "Quality gate", Type: domain.NodeQualityGate, InputSchema: envelope, OutputSchema: envelope, QualityGate: &domain.QualityGateNodeConfig{GateName: "release", Blocking: true}},
		{ID: "publish", Name: "Publish", Type: domain.NodePublish, InputSchema: envelope, OutputSchema: envelope, Publish: &domain.PublishNodeConfig{Environment: "production", RequiredRole: "owner", AllowRollback: true}},
	}
	edgePairs := [][2]string{
