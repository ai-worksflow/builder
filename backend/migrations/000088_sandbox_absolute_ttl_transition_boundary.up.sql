...
BEGIN
  ...
  IF NOT FOUND OR parent.version <> expected_version OR parent.session_epoch <> expected_session_epoch THEN
    RAISE EXCEPTION 'expired SandboxSession transition CAS or epoch precondition failed'
      USING ERRCODE = '40001';
  END IF;
  ...
  IF transition_reason IS NULL OR length(transition_reason) NOT BETWEEN 1 AND 2000 THEN
    RAISE EXCEPTION 'expired SandboxSession transition reason is required and bounded'
      USING ERRCODE = '22023';
  END IF;

  IF target_actor_id <> '00000000-0000-4000-8000-000000000047'::uuid
     OR parent.expires_at > statement_timestamp() THEN
    RAISE EXCEPTION 'expired SandboxSession transition requires the lifecycle worker after absolute TTL'
      USING ERRCODE = '42501';
  END IF;

  IF parent.state IN ('ready', 'suspended', 'failed') AND target_state = 'terminating' THEN
  ...
  ELSE
    RAISE EXCEPTION 'invalid expired SandboxSession lifecycle transition'
      USING ERRCODE = '55000';
  END IF;
  ...
  INSERT INTO sandbox_session_transition_events (
    ...
    parent.id, parent.version, parent.version, parent.version + 1,
    parent.state, target_state, parent.session_epoch, parent.session_epoch,
    event_kind, target_actor_id, transition_reason,
    parent.candidate_version, parent.candidate_version,
    parent.candidate_journal_sequence, parent.candidate_journal_sequence,
    parent.candidate_session_epoch, parent.candidate_session_epoch,
    parent.candidate_writer_lease_epoch, parent.candidate_writer_lease_epoch,
    parent.candidate_tree_store, parent.candidate_tree_store,
    parent.candidate_tree_owner_id, parent.candidate_tree_owner_id,
    parent.candidate_tree_ref, parent.candidate_tree_ref,
    parent.candidate_tree_content_hash, parent.candidate_tree_content_hash,
    parent.candidate_tree_hash, parent.candidate_tree_hash,
    parent.candidate_dirty, parent.candidate_dirty,
    parent.candidate_conflicted, parent.candidate_conflicted,
    parent.candidate_stale, parent.candidate_stale,
    parent.candidate_rebase_required, parent.candidate_rebase_required,
