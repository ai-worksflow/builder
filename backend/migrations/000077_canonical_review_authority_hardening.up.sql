-- Preserve migration 000076 as an immutable published contract. This
-- follow-up installs the reviewed Canonical Review hardening that was authored
-- after 000076 had already been applied.

-- Hold the complete source/receipt writer boundary for the whole migration.
-- No 000076 receipt may appear between compatibility preflight and installing
-- the hardened append-only/issuer definitions.
...
AS $function$
  SELECT p_value ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
     AND p_value <> '00000000-0000-0000-0000-000000000000'
$function$;
...
-- A receipt published by 000076 may have been valid under its earlier
-- verifier while violating the new exact predecessor/primitive contract.
-- Never turn issued durable proof into unreadable state. This migration is
-- transactional and the writer locks above make the preflight race-free; a
-- failure restores the complete 000076 catalog and authority unchanged.
DO $canonical_review_hardening_compatibility_preflight$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'cannot apply Canonical Review hardening: an existing 000076 receipt is incompatible'
      USING ERRCODE = 'WCR02';
  END IF;
END;
$canonical_review_hardening_compatibility_preflight$;
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION 'Review decisions are append-only' USING ERRCODE = 'WCR02';
  END IF;
...
DO $canonical_review_authority_hardening_security$
DECLARE
  schema_name text := current_schema();
  role_name text;
BEGIN
  EXECUTE format('ALTER FUNCTION %I.canonical_review_uuid_is_exact(text) SET search_path TO pg_catalog', schema_name);
  EXECUTE format('ALTER FUNCTION %I.canonical_review_text_is_trimmed(text) SET search_path TO pg_catalog', schema_name);
  EXECUTE format('ALTER FUNCTION %I.canonical_review_timestamp_is_exact(text) SET search_path TO pg_catalog', schema_name);
  ...
  EXECUTE format('REVOKE ALL ON FUNCTION %I.canonical_review_uuid_is_exact(text) FROM PUBLIC', schema_name);
  EXECUTE format('REVOKE ALL ON FUNCTION %I.canonical_review_text_is_trimmed(text) FROM PUBLIC', schema_name);
  EXECUTE format('REVOKE ALL ON FUNCTION %I.canonical_review_timestamp_is_exact(text) FROM PUBLIC', schema_name);

  IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'worksflow_migration_owner') THEN
    EXECUTE format('ALTER FUNCTION %I.canonical_review_uuid_is_exact(text) OWNER TO worksflow_migration_owner', schema_name);
    EXECUTE format('ALTER FUNCTION %I.canonical_review_text_is_trimmed(text) OWNER TO worksflow_migration_owner', schema_name);
    EXECUTE format('ALTER FUNCTION %I.canonical_review_timestamp_is_exact(text) OWNER TO worksflow_migration_owner', schema_name);
  END IF;
  ...
  FOREACH role_name IN ARRAY ARRAY[
    ...
    IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = role_name) THEN
      EXECUTE format('REVOKE ALL ON FUNCTION %I.canonical_review_uuid_is_exact(text) FROM %I', schema_name, role_name);
      EXECUTE format('REVOKE ALL ON FUNCTION %I.canonical_review_text_is_trimmed(text) FROM %I', schema_name, role_name);
      EXECUTE format('REVOKE ALL ON FUNCTION %I.canonical_review_timestamp_is_exact(text) FROM %I', schema_name, role_name);
    END IF;
  END LOOP;
END;
$canonical_review_authority_hardening_security$;

COMMENT ON FUNCTION canonical_review_uuid_is_exact(text) IS
  'Closed UUID identity predicate used by Canonical Review authority.';
COMMENT ON FUNCTION canonical_review_text_is_trimmed(text) IS
  'Unicode-aware boundary whitespace predicate used by Canonical Review authority.';
