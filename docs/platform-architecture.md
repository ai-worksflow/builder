...
资格控制面的后续迁移顺序固定为 `000080` Qualification Input Precommit
Authority → `000081` Promotion v2 → `000082` private Handoff。`000080` 用两个
互不混用的 source-verifier/credential-resolver admission 把当前 WIA、Policy 与 Plan
组成一个 append-only 权威；`000081` 必须把它的完整 `PromotionBinding` 作为 canonical
closure 中固定必需的 typed `inputPrecommit` 成员，而不能塞入 Policy 可配置的
independent-authority 列表或只留一个旁路外键。`000082` 只能消费该新版 Promotion
产生的 pending handoff。Promotion runtime 必须持续禁用，直到 `000080` 的 SQL/隔离
角色/no-bypass canary 与 `000081` 的更新 consume canary 都通过；任何单独 posture
结果都不是激活授权。
...
14. 外部资格链必须严格按 `000080` 输入预提交、`000081` Promotion、`000082` Handoff 前进；Promotion closure 直接认证完整 typed `inputPrecommit`，三阶段各自的 SQL/角色/真实 PostgreSQL/no-bypass canary 均通过且 runtime activation 另行批准前，Workflow 批准与 Publish 都保持阻塞。
