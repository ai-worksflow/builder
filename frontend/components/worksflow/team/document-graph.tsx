...
export function DocumentGraph() {
  ...
  const loadGraph = useCallback(async () => {
    ...
    try {
      ...
      if (sequence.current === request) {
        setGraph(null)
        setGraphError(collaborationErrorMessage(error, t('teamPlatform.graph.loadFailed')))
      }
    } finally {
      if (sequence.current === request) setLoading(false)
    }
  }, [platformClient.documents, project, session.signedIn, t])
  ...
  async function createDependency() {
    ...
    try {
      ...
      await loadGraph()
      setTargetNodeId('')
      setNotice({ kind: 'success', message: t('teamPlatform.graph.dependencyCreated') })
    } catch (error) {
      setNotice({ kind: 'error', message: collaborationErrorMessage(error, t('teamPlatform.graph.dependencyFailed')) })
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="flex h-full max-lg:flex-col max-lg:overflow-y-auto" data-testid="server-document-graph">
      <div ref={scrollFrameRef} className="relative flex-1 overflow-auto bg-canvas scrollbar-thin max-lg:min-h-[560px] max-lg:flex-none">
        <div className="sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-border bg-surface/90 px-5 py-3 backdrop-blur max-md:flex-wrap max-md:px-4">
          <div className="flex min-w-0 items-center gap-2"><Workflow className="size-4 shrink-0 text-primary-bright" /><span className="text-sm font-semibold text-foreground">{t('graph.title')}</span><span className="truncate text-xs text-faint-foreground">{project?.name ?? t('teamPlatform.graph.noProject')}</span><span className="shrink-0 rounded border border-border bg-surface-2 px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-faint-foreground">{t('teamPlatform.graph.serverProjection')}</span></div>
          <div className="flex max-w-full items-center gap-2 overflow-x-auto scrollbar-thin">
            {(['all', 'artifacts', 'ai', 'workflow', 'delivery'] as const).map((id) => <button key={id} type="button" onClick={() => setNodeFilter(id)} className={cn('shrink-0 rounded-md px-2 py-1 text-[11px] font-medium', nodeFilter === id ? 'bg-primary/15 text-primary-bright' : 'text-faint-foreground hover:bg-white/5')}>{nodeFilterLabel(id, t)}</button>)}
            ...
            <button type="button" onClick={() => void loadGraph()} disabled={loading} className="inline-flex shrink-0 items-center gap-1 rounded-md border border-border px-2 py-1 text-[11px] text-muted-foreground disabled:opacity-50"><RefreshCw className={cn('size-3', loading && 'animate-spin')} />{t('preview.refresh')}</button>
          </div>
        </div>

        {loading && !graph ? <GraphMessage icon={<RefreshCw className="size-7 animate-spin text-primary-bright" />} title={t('teamPlatform.graph.loadingTitle')} body={t('teamPlatform.graph.loadingDetail')} />
          : graphError ? <GraphMessage icon={<ShieldAlert className="size-7 text-destructive" />} title={t('teamPlatform.graph.loadErrorTitle')} body={graphError} />
            : nodes.length === 0 ? <GraphMessage icon={<GitFork className="size-8 text-primary-bright" />} title={t('graph.emptyTitle')} body={t('teamPlatform.graph.noFilterMatches')} />
              : <div className="relative" style={{ width: canvasWidth, height: canvasHeight, backgroundSize: '28px 28px' }}>
                <div className="absolute left-0 right-0 top-4 grid grid-cols-4 gap-[70px] px-10 text-[10px] font-semibold uppercase tracking-[0.16em] text-faint-foreground"><ColumnHeading icon={<FileText className="size-3" />} label={t('teamPlatform.graph.column.artifacts')} count={rowsByColumn[0]} locale={locale} /><ColumnHeading icon={<Bot className="size-3" />} label={t('teamPlatform.graph.column.frozenInput')} count={rowsByColumn[1]} locale={locale} /><ColumnHeading icon={<Boxes className="size-3" />} label={t('teamPlatform.graph.column.reviewOutput')} count={rowsByColumn[2]} locale={locale} /><ColumnHeading icon={<Rocket className="size-3" />} label={t('teamPlatform.graph.column.delivery')} count={rowsByColumn[3]} locale={locale} /></div>
                ...
                {nodes.map((node) => <button key={node.id} type="button" data-testid={`document-graph-node-${node.entityType}`} onClick={() => { setSelectedNodeId(node.id); setNotice(null) }} onDoubleClick={() => openArtifact(node)} aria-label={t('teamPlatform.graph.nodeLabel', { kind: kindLabel(node.entityType, t), title: node.title, status: statusLabel(node.status, t) })} className={cn('absolute rounded-lg border bg-surface-2 p-3 text-left shadow-sm outline-none', selectedNode?.id === node.id ? 'border-primary shadow-[0_0_0_1px_var(--color-primary)]' : 'border-border hover:border-white/25')} style={{ left: node.x, top: node.y, width: NODE_WIDTH, height: NODE_HEIGHT }}><div className="flex items-center justify-between gap-2"><span className="flex min-w-0 items-center gap-1.5 text-[10px] font-medium uppercase tracking-wide text-faint-foreground"><span className={cn('size-1.5 shrink-0 rounded-full', statusDot(node.status))} /><span className="truncate">{kindLabel(node.entityType, t)}</span></span><span className="max-w-24 truncate rounded border border-border px-1 py-.5 text-[9px] text-faint-foreground">{statusLabel(node.status, t)}</span></div><div className="mt-1 truncate text-sm font-semibold text-foreground">{node.title}</div><div className="mt-1 line-clamp-2 min-h-7 text-[10px] leading-3.5 text-muted-foreground">{nodeSummary(node, t, locale)}</div><div className="mt-1.5 flex items-center justify-between gap-2 text-[9px] text-faint-foreground"><code className="truncate">{node.revision?.contentHash?.slice(0, 15) ?? node.entityId.slice(0, 12)}</code>{Boolean(node.memberBindings?.length) && <span className="shrink-0">{t('teamPlatform.graph.bindingsCount', { count: (node.memberBindings?.length ?? 0).toLocaleString(locale) })}</span>}</div></button>)}
              </div>}
      </div>
      ...
      <aside className="flex w-80 shrink-0 flex-col border-l border-border bg-surface max-lg:max-h-[560px] max-lg:w-full max-lg:border-l-0 max-lg:border-t">
        {selectedNode ? <><div className="border-b border-border p-4"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-semibold uppercase tracking-wider text-faint-foreground">{kindLabel(selectedNode.entityType, t)}</span><StatusBadge status={selectedNode.status} /></div><h3 className="mt-1 text-base font-semibold text-foreground">{selectedNode.title}</h3><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{nodeSummary(selectedNode, t, locale)}</p><dl className="mt-3 grid grid-cols-[84px_1fr] gap-x-2 gap-y-1 text-[10px]"><dt className="text-faint-foreground">{t('teamPlatform.graph.entity')}</dt><dd className="truncate font-mono text-muted-foreground">{selectedNode.entityId}</dd><dt className="text-faint-foreground">{t('teamPlatform.graph.revision')}</dt><dd className="truncate font-mono text-muted-foreground">{selectedNode.revision?.revisionId ?? t('teamPlatform.graph.notRevision')}</dd></dl>{selectedNode.id.startsWith('artifact:') && selectedNode.artifactKind !== 'workspace' && <button type="button" onClick={() => openArtifact(selectedNode)} className="mt-3 inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-md border border-primary/35 bg-primary/10 px-2 text-[10px] font-semibold text-primary-bright"><PencilLine className="size-3.5" />{t('teamPlatform.graph.openWorkspace')}</button>}{notice && <div className={cn('mt-3 rounded-md border px-2.5 py-2 text-[11px]', notice.kind === 'success' ? 'border-success/30 bg-success/10 text-success' : 'border-destructive/30 bg-destructive/10 text-destructive')}>{notice.message}</div>}</div>
          <div className="flex-1 overflow-y-auto p-4 scrollbar-thin">
            {Boolean(selectedNode.memberBindings?.length) && <><SectionLabel>{t('teamPlatform.graph.memberBindings')}</SectionLabel><div className="mt-2 space-y-1.5">{selectedNode.memberBindings?.map((binding) => <div key={`${binding.userId}:${binding.role}`} className="rounded border border-border bg-surface-2 p-2 text-[10px]"><span className="font-medium text-foreground">{members.find((member) => member.user.id === binding.userId)?.user.name ?? binding.userId}</span><span className="float-right text-primary-bright">{relationLabel(binding.role, t)}</span>{binding.reason && <p className="mt-1 text-faint-foreground">{binding.reason}</p>}</div>)}</div></>}
            {selectedNode.id.startsWith('artifact:') && selectedNode.revision && <><SectionLabel className="mt-5">{t('teamPlatform.graph.createPinnedDependency')}</SectionLabel><div className="mt-2 space-y-2 rounded-lg border border-border bg-surface-2 p-3"><select value={targetNodeId} onChange={(event) => setTargetNodeId(event.target.value)} aria-label={t('teamPlatform.graph.targetArtifact')} className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-xs text-foreground"><option value="">{t('teamPlatform.graph.chooseTarget')}</option>{availableTargets.map((node) => <option key={node.id} value={node.id}>{node.title}</option>)}</select><select value={relation} onChange={(event) => setRelation(event.target.value as DependencyRelation)} aria-label={t('graph.relationType')} className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-xs text-foreground">{RELATIONS.map((item) => <option key={item} value={item}>{relationLabel(item, t)}</option>)}</select><label className="flex items-center gap-2 text-[11px] text-muted-foreground"><input type="checkbox" checked={required} onChange={(event) => setRequired(event.target.checked)} />{t('teamPlatform.graph.requiredDelivery')}</label><button type="button" onClick={() => void createDependency()} disabled={!can('edit') || !targetNodeId || saving} className="w-full rounded-md bg-primary px-2.5 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-40">{saving ? t('teamPlatform.graph.creating') : t('teamPlatform.graph.createDependency')}</button></div></>}
            <SectionLabel className="mt-5">{t('teamPlatform.graph.exactRelationships')}</SectionLabel><div className="mt-2 space-y-1.5">{selectedEdges.map((edge) => { const outgoing = edge.sourceId === selectedNode.id; const other = nodeById.get(outgoing ? edge.targetId : edge.sourceId); if (!other) return null; return <button key={edge.id} type="button" onClick={() => setSelectedNodeId(other.id)} className="flex w-full items-center gap-2 rounded-lg border border-border bg-surface-2 px-2.5 py-2 text-left"><Link2 className="size-3.5 shrink-0 text-primary-bright" /><span className="min-w-0 flex-1"><span className="block truncate text-[10px] text-faint-foreground">{outgoing ? '' : '← '}{relationLabel(edge.relation, t)}</span><span className="flex items-center gap-1 truncate text-xs font-medium text-foreground">{outgoing && <ArrowRight className="size-3" />}{other.title}</span></span>{edge.required && <span className="rounded bg-destructive/10 px-1 py-.5 text-[9px] text-destructive">{t('blueprint.required')}</span>}</button>})}{selectedEdges.length === 0 && <p className="text-xs text-faint-foreground">{t('teamPlatform.graph.noRelationships')}</p>}</div>
            <SectionLabel className="mt-5">{t('teamPlatform.graph.serverMetadata')}</SectionLabel><pre className="mt-2 max-h-48 overflow-auto rounded border border-border bg-background p-2 text-[9px] text-faint-foreground">{JSON.stringify(selectedNode.metadata, null, 2)}</pre>
...
function nodeSummary(node: DocumentGraphNodeDto, t: Translate, locale: string) {
  if (node.entityType === 'inputManifest') return t('teamPlatform.graph.summary.frozenSources', { job: String(node.metadata.jobType ?? t('teamPlatform.graph.aiInput')), count: formatCount(node.metadata.sourceCount, locale) })
  if (node.entityType === 'outputProposal') return t('teamPlatform.graph.summary.operationsPending', { operations: formatCount(node.metadata.operationCount, locale), pending: formatCount(node.metadata.pendingCount, locale) })
  if (node.entityType === 'deployment') return `${String(node.metadata.environment ?? '')} · ${String(node.metadata.previewUrl ?? t('teamPlatform.graph.noPreviewUrl'))}`
  if (node.entityType === 'workbenchVersion') return t('teamPlatform.graph.summary.manifest', { hash: String(node.metadata.manifestHash ?? '').slice(0, 16) })
  if (node.entityType === 'implementation') return t('teamPlatform.graph.summary.implementationOperations', { count: formatCount(node.metadata.operationCount, locale) })
  return node.memberBindings?.length
    ? t('teamPlatform.graph.summary.memberBindings', { count: node.memberBindings.length.toLocaleString(locale) })
    : t('teamPlatform.graph.summary.serverState', { kind: kindLabel(node.entityType, t) })
}

function kindLabel(kind: DocumentGraphEntityType, t: Translate) {
  const labels: Record<DocumentGraphEntityType, string> = {
    document: t('common.document'),
    feature: t('teamPlatform.graph.kind.feature'),
    page: t('teamPlatform.graph.kind.page'),
    api: t('blueprint.node.api'),
    data: t('teamPlatform.graph.kind.data'),
    workspace: t('teamPlatform.graph.kind.workspace'),
    inputManifest: t('teamPlatform.graph.kind.inputManifest'),
    outputProposal: t('teamPlatform.graph.kind.outputProposal'),
    workflowRun: t('teamPlatform.graph.kind.workflowRun'),
    workbenchVersion: t('teamPlatform.graph.kind.workbenchVersion'),
    implementation: t('teamPlatform.graph.kind.implementation'),
    deployment: t('teamPlatform.graph.kind.deployment'),
  }
  return labels[kind]
}
...
function relationLabel(value: string, t: Translate) {
  const labels: Record<string, string> = {
    owner: t('role.owner'),
    assignee: t('role.assignee'),
    downstreamOwner: t('role.downstreamOwner'),
    reviewer: t('role.reviewer'),
    watcher: t('role.watcher'),
    ...
    generated_output: t('teamPlatform.graph.relation.generatedOutput'),
    proposes_patch: t('teamPlatform.graph.relation.proposesPatch'),
    frozen_input: t('teamPlatform.graph.relation.frozenInput'),
    proposal_base: t('teamPlatform.graph.relation.proposalBase'),
    deployed_as: t('teamPlatform.graph.relation.deployedAs'),
  }
...
function statusLabel(status: string, t: Translate) {
  const labels: Record<string, string> = {
    ...
    ready: t('teamPlatform.common.ready'),
    ...
    running: t('teamPlatform.graph.status.running'),
    frozen: t('teamPlatform.graph.status.frozen'),
    pending: t('teamPlatform.editor.proposalDecision.pending'),
    blocked: t('teamPlatform.graph.status.blocked'),
    stale: t('teamPlatform.graph.status.stale'),
    invalid: t('teamPlatform.graph.status.invalid'),
    succeeded: t('teamPlatform.graph.status.succeeded'),
    failed: t('teamPlatform.graph.status.failed'),
    deployed: t('teamPlatform.graph.status.deployed'),
  }
  return labels[status] ?? status
}

function nodeFilterLabel(filter: NodeFilter, t: Translate) {
  const labels: Record<NodeFilter, string> = {
    all: t('graph.filter.all'),
    artifacts: t('teamPlatform.graph.filter.artifacts'),
    ai: t('teamPlatform.graph.filter.ai'),
    workflow: t('teamPlatform.graph.filter.workflow'),
    delivery: t('teamPlatform.graph.filter.delivery'),
  }
...
function formatCount(value: unknown, locale: string) {
  ...
  return Number.isFinite(count) ? count.toLocaleString(locale) : String(value ?? 0)
}
...
function StatusBadge({ status }: { status: string }) {
  const { t } = useI18n()
  return <span className="rounded border border-border bg-white/5 px-1.5 py-.5 text-[9px] font-medium text-muted-foreground">{statusLabel(status, t)}</span>
}
function ColumnHeading({ icon, label, count, locale }: { icon: React.ReactNode; label: string; count: number; locale: string }) { return <div className="flex w-[224px] items-center gap-1.5 border-b border-border pb-2">{icon}<span>{label}</span><span className="ml-auto rounded bg-white/5 px-1.5 py-.5 text-[9px]">{count.toLocaleString(locale)}</span></div> }
