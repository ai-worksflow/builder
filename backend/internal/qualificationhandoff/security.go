...
func forbiddenSecretString(value string) bool {
	...
	return privateKeyPattern.MatchString(folded) || providerTokenPattern.MatchString(folded) ||
		jwtPattern.MatchString(value) || credentialURLPattern.MatchString(folded) ||
		credentialAssignmentPattern.MatchString(folded) || headerValuePattern.MatchString(folded) ||
		bearerPattern.MatchString(folded) || environmentAssignmentPattern.MatchString(folded) ||
		absoluteHostPathPattern.MatchString(folded)
}
