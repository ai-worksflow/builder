-- Restore the exact live 000077 authority definitions observed before 000083.
...
-- Migration 83 is an owner-executed bridge over historical Release helper
-- ownership.  Remove only grants whose provenance row proves that 83 added
-- them; a pre-existing effective migration-owner capability is preserved.
DO $canonical_review_83_legacy_release_acl_down$
DECLARE
...
BEGIN
  ...
  THEN
    FOR v_signature IN
      ...
      FROM canonical_review_83_legacy_release_acl_provenance
  ...
  END IF;
END;
$canonical_review_83_legacy_release_acl_down$;
...
BEGIN
  ...
  IF jsonb_typeof(v_value->'soloSelfReviewOwnerId') = 'string'
     AND (v_value->>'governanceMode' <> 'solo'
       OR v_governance->>'soleOwnerId' <> v_value->>'soloSelfReviewOwnerId'
  ...
  LOOP
    ...
    IF jsonb_typeof(v_decision) <> 'object'
       ...
       OR v_decision->>'createdAt' !~ v_time_pattern
    ...
    IF v_facts - ARRAY['explicitConfirmation','governanceMode','ownerCount','preconditionETag','reviewerRole','soleOwnerId','version'] <> '{}'::jsonb
       ...
       OR v_facts->>'preconditionETag' <> v_expected_precondition
     ...
     OR EXISTS (
       ...
       WHERE decision.review_request_id = v_request.id
         AND (decision.review_authority_version <> 1
           ...
           OR decision.precondition_etag <> format(
             '"review:%s:open:%s:%s"',
             v_request.id::text,
             history.prior_count,
...
DO $canonical_review_83_restore_search_path$
DECLARE
...
END;
$canonical_review_83_restore_search_path$;
