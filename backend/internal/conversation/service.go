...
type conversationStore interface {
	...
	CreateIntentProposal(context.Context, uuid.UUID, uuid.UUID, uuid.UUID, CreateIntentProposalInput, ProposalProvenance) (WorkflowIntentProposal, Message, error)
	IntentGenerationContext(context.Context, uuid.UUID, uuid.UUID, uuid.UUID, []uuid.UUID, []platformdomain.ArtifactRef, ManifestIntent) (intentGenerationContext, error)
...
func (s *Service) createIntentProposal(ctx context.Context, projectID, conversationID, actorID string, input CreateIntentProposalInput, provenance ProposalProvenance) (WorkflowIntentProposal, Message, error) {
	...
	if err := s.validateManifestSources(ctx, projectID, input.ManifestIntent, input.SourceRefs); err != nil {
		return WorkflowIntentProposal{}, Message{}, err
	}
	...
	return s.store.CreateIntentProposal(ctx, projectUUID, conversationUUID, actorUUID, input, provenance)
}
...
func (s *Service) GenerateIntentProposal(ctx context.Context, projectID, conversationID, actorID string, input GenerateIntentProposalInput) (GeneratedIntentProposal, error) {
	...
	if err := s.validateManifestSources(ctx, projectID, validationInput.ManifestIntent, validationInput.SourceRefs); err != nil {
		return GeneratedIntentProposal{}, err
	}
	generationContext, err := s.store.IntentGenerationContext(
		ctx, projectUUID, conversationUUID, triggerID, candidates, validationInput.SourceRefs, validationInput.ManifestIntent,
	)
	if err != nil {
		return GeneratedIntentProposal{}, err
	}
	...
	schema, err := intentProposalSchema(input.CandidateDefinitionVersionIDs)
	if err != nil {
		return GeneratedIntentProposal{}, err
	}
	generated, err := s.provider.Generate(ctx, ai.Request{
		RunID: input.TriggerMessageID, Model: strings.TrimSpace(input.Model),
		Instructions: "Select one supplied immutable workflow definition. Return a reviewable intent proposal only. Never claim that a workflow, artifact, bundle, or application was changed. Preserve the supplied exact source and manifest identities; propose scope and a concrete Workbench instruction for a human to accept or reject. Choose workbench_instruction only when the conversation provides an existing exact workflow run and then set workbenchInstruction.expectedRunId; otherwise choose start_workflow. Do not add a conversationIntent property to scope; the server injects that reserved reviewed instruction envelope.",
...
func (s *Service) ExecuteCommand(ctx context.Context, projectID, conversationID, commandID, actorID, expectedETag string, input ExecuteCommandInput) (ConversationCommand, error) {
	...
	case IntentWorkbenchInstruction:
		...
		if err := s.store.ValidateWorkbenchResult(ctx, projectUUID, current.Payload, *input.WorkbenchResult); err != nil {
			return ConversationCommand{}, err
		}
	default:
	...
	if claim.Command.Kind == IntentWorkbenchInstruction {
		result, _ := platformdomain.CanonicalJSON(input.WorkbenchResult)
...
func (s *Service) validateManifestSources(ctx context.Context, projectID string, intent ManifestIntent, refs []platformdomain.ArtifactRef) error {
	...
	if manifest.ProjectID != projectID || manifest.Ref() != intent.InputManifest {
		return platformdomain.ErrManifestUnpinned
	}
...
func intentProposalSchema(candidateIDs []string) (json.RawMessage, error) {
	return platformdomain.CanonicalJSON(map[string]any{
		...
		"properties": map[string]any{
			...
			"kind":                         map[string]any{"type": "string", "enum": []string{string(IntentStartWorkflow), string(IntentWorkbenchInstruction)}},
			"suggestedDefinitionVersionId": map[string]any{"type": "string", "enum": candidateIDs},
