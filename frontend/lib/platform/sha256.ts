...
function portableSha256(source: Uint8Array) {
...
/**
 * Computes SHA-256 with Web Crypto when it is available and uses the same
 * byte-level algorithm in insecure browser contexts where SubtleCrypto is
...
export async function sha256Bytes(
  ...
  if (subtle) {
    try {
      ...
      // Fall through to the portable implementation. Integrity verification
      // remains mandatory even if the host's crypto provider is unavailable.
    }
  }
  return portableSha256(bytes)
}

export function sha256DigestString(digest: Uint8Array) {
