...
`qualification-promotion-operator.md`，不把仓库内单元测试、fixture、普通
...
Promotion consumption v2
  -> re-locks exact Plan, Evidence ledger, Receipt and current canonical review
  -> atomically creates one pending immutable-revision handoff

workflow handoff consumer
  -> creates the exact immutable revision and submits the node under CAS once
```

Golden Authority 与 Fixture 是 root-owned、hash-bound 输入，不伪装成独立签名
结论。Plan Authority、credential issuer、KMS、runner、approver、Promotion v2
消费账本与 workflow handoff consumer 共同组成信任链；各角色不得复用同一公钥
或身份。历史 wire-v2 Promotion Authority/Receipt/快照顺序只允许只读验证，不能
作为新流程的写入协议。
...
reviewed source + qualification-ready revision
  ...
  -> promotion transaction CAS-consumes target + nonce + authority digest once
     and appends one hash-bound pending handoff with a preallocated revision ID
  -> workflow transaction rechecks target/upstream/review/independent gates,
     creates the promotion-only immutable revision and submits the exact node
```
...
和强负向向量内部实现。内部 atomic consume/pending-handoff 数据面也已经实现，但尚未在
approved target 执行，没有形成加密 artifact、外部签名 Receipt、真实 consume ledger 记录
或 workflow-side handoff completion，因此仍不是 external qualification evidence。
...
- qualification-evidence 的严格 lifecycle、opaque Plan Authority `Execute` 边界，以及
  ...
  Store 已通过真实 PostgreSQL 并发/replay/raw-hash/rollback canary；但没有生产
...
- 下游 exact target/nonce/authority append-only consumption ledger 与 pending handoff 已有
  内部实现，但尚无外部运行证据，也没有把 handoff 原子转换为 immutable revision + node
  submit 的 workflow consumer。

因此实现阶段可以继续完善 snapshot-first wire v3 Store、orchestrator 与 verifier，但在真实外部运行
产生、验证并单次消费 Receipt 之前，manifest 必须保持 `status: not-qualified`，Workflow
批准按钮必须保持阻塞。
...
7. 在 approved target 执行 qualification，生成 immutable evidence；
8. 由独立 verifier 与 consume ledger 生成 pending handoff，再由 workflow consumer 完成
   promotion-only immutable revision 与 exact node submit；
9. 最后再分别完成 ModelProfile 与 production PostgreSQL 独立治理门禁。
